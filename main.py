...
while 1:
	...
	for i in event.get():
      ...
      if i.key == K_r:
        if i.key == K_r:
				#обнуляем переменные killed,passed
				killed=0
				passed=0
				#заново рендерим надписи text_killed,text_passed
				text_killed = font24.render("Убито:"+str(killed),True,(255,0,0))
				text_passed = font24.render("Пропущенно:"+str(passed),True,(0,255,0))
				#ракету помещаем на х=250 у=450
				rocket.rect.x=250
				rocket.rect.y=450
				bullets.empty()#очистили группу
				ufos.empty()#очистили группу
				for i in range(5):
					ufo = UFO(width=50, height=50, img="ufo.png", x=randint(0, win_wid-50), y=0, speed=1)# 
					ufos.add(ufo)#добавили в группу
				game = "in_process" 
